...
if [ ! -f "segmentation/polygon_generator.py" ]; then
    print_error "segmentation/polygon_generator.py not found. Please run from project root."
    exit 1
fi
...
if python3 -m pytest tests/test_polygon_generator.py -v; then
...
if python3 -m pytest tests/test_integration.py -v; then
...
cd "$PROJECT_ROOT/phaser-matter-game"
...
.venv/bin/python3 segmentation/polygon_generator.py segmentation/polygon_config/rectangle.json &
PYTHON_PID=$!
...
if .venv/bin/python3 tests/test_pipeline_fps.py; then
