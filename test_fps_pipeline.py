#!/usr/bin/env python3
"""
Full Pipeline FPS Test
Tests the complete dual-process architecture to ensure 60 FPS performance.
"""
...
class FPSTestPipeline:
    """Test the full pipeline FPS performance."""
    ...
    async def start_python_generator(self):
        """Start the Python rectangle generator."""
        ...
        if self.python_process.poll() is not None:
            ...
            print(f"❌ Python generator failed to start:")
        ...
        try:
            # Non-blocking read to check if server started
            import select
            if select.select([self.python_process.stdout], [], [], 0.1)[0]:
                output = self.python_process.stdout.readline()
                if "Starting rectangle generator server" in output:
                    print("✅ Python generator started successfully")
                    return True
        except:
            pass
    ...
    async def start_node_game(self):
        """Start the Node.js/TypeScript game."""
        print("🎮 Starting TypeScript game...")
        
        # Change to game directory and start
        game_dir = "phaser-matter-game"
        ...
        if self.node_process.poll() is not None:
            ...
            print(f"❌ Node game failed to start:")
            print(f"STDOUT: {stdout}")
            print(f"STDERR: {stderr}")
            return False
            
        print("✅ TypeScript game started successfully")
        return True
    ...
    async def test_websocket_fps(self, duration_seconds: int = 5) -> Dict[str, float]:
        ...
        for attempt in range(5):
            try:
                print(f"   Attempt {attempt + 1}/5: Connecting to WebSocket...")
                websocket = await websockets.connect("ws://localhost:8765", timeout=2)
    ...
    def cleanup(self):
        ...
        if self.python_process:
            try:
                ...
                print(f"Warning: Error stopping Python process: {e}")
        ...
        if self.node_process:
            try:
                ...
                print(f"Warning: Error stopping Node process: {e}")
    ...
    async def run_full_test(self, duration_seconds: int = 5) -> Dict[str, Any]:
        ...
        try:
            ...
            if not await self.start_python_generator():
                return {'error': 'Failed to start Python generator'}
...
def print_results(results: Dict[str, Any]):
    ...
    print("\n" + "=" * 50)
    print("📊 FPS PIPELINE TEST RESULTS")
    print("=" * 50)
